...
namespace uPLibrary.Networking.M2MqttClient
{
    ...
    public class MqttClient
    {
        ...
        private QueueTS<MqttMsgContext> inflightQueue;
        // internal queue for received messages about inflight messages
        private QueueTS<MqttMsgBase> internalQueue;
        // internal queue for dispatching events
        private QueueTS<InternalEvent> eventQueue;
        // session
        ...
        public MqttClient(IPAddress brokerIpAddress) :
            this(brokerIpAddress, MqttSettings.MQTT_BROKER_DEFAULT_PORT, false, null, null, SslProtocols.None)
        {
        }
        ...
        public MqttClient(IPAddress brokerIpAddress, int brokerPort, bool secure, X509Certificate caCert, X509Certificate clientCert, SslProtocols sslProtocol)
        {
...
#if !(WINDOWS_APP || WINDOWS_PHONE_APP)
            this(brokerHostName, MqttSettings.MQTT_BROKER_DEFAULT_PORT, false, null, null, SslProtocols.None)
#else
            this(brokerHostName, MqttSettings.MQTT_BROKER_DEFAULT_PORT, false, SslProtocols.None)
#endif
        {
        }
        ...
        private void Init(string brokerHostName, int brokerPort, bool secure, X509Certificate caCert, X509Certificate clientCert, SslProtocols sslProtocol,
            RemoteCertificateValidationCallback userCertificateValidationCallback,
            LocalCertificateSelectionCallback userCertificateSelectionCallback)
#elif (WINDOWS_APP || WINDOWS_PHONE_APP)
...
#endif
        {
            ...
            // queue for handling inflight messages (publishing and acknowledge)
            this.inflightWaitHandle = new AutoResetEvent(false);
            this.inflightQueue = new QueueTS<MqttMsgContext>();

            // queue for received message
            this.receiveEventWaitHandle = new AutoResetEvent(false);
            this.eventQueue = new QueueTS<InternalEvent>();
            this.internalQueue = new QueueTS<MqttMsgBase>();
        ...
        private bool EnqueueInflight(MqttMsgBase msg, MqttMsgFlow flow)
        {
                ...
                (msg.QosLevel == MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE))
            {
                ...
                MqttMsgContext msgCtx = this.inflightQueue.Get(msgCtxFinder.Find);
        ...
        private void ReceiveThread()
        {
            ...
            while (this.isRunning)
            {
                ...
                {
#if TRACE
                    MqttUtility.Trace.WriteLine(TraceLevel.Error, "{0}: clientid: {1} Exception occurred: {2}", DateTime.Now, this.ClientId, e.ToString());
#endif
        ...
        private void DispatchEventThread()
        {
            while (this.isRunning)
            {
                ...
                if (this.isRunning)
                {
                    ...
                    internalEvent = this.eventQueue.TryDequeueOrDefault();
                ...
                while (this.isRunning)
                {
                    ...
                    if (this.isRunning)
                    {
                        ...
                        while (count > 0)
                        {
                            ...
                            msgContext = this.inflightQueue.Dequeue();
                            ...
                            switch (msgContext.State)
                            {
                                ...
                                case MqttMsgState.WaitForUnsuback:
                                    ...
                                    {
                                        acknowledge = false;
                                        msgReceived = this.internalQueue.TryPeek();
